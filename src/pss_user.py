...
async def get_user_details_by_info(
    ctx: Context, user_info: EntityInfo, max_tourney_battle_attempts: int = None, retrieved_at: datetime = None, past_fleet_infos: EntitiesData = None, as_embed: bool = settings.USE_EMBEDS
) -> Union[List[Embed], List[str]]:
    ...
    user_details = __create_user_details_from_info(
        user_info, fleet_info, ship_info, max_tourney_battle_attempts=max_tourney_battle_attempts, retrieved_at=retrieved_at, is_past_data=is_past_data, is_in_tourney_fleet=is_in_tourney_fleet
    )
...
async def __get_users_data(user_name_or_id: str) -> EntitiesData:
    ...
    try:
        ...
        pass
    if user_id:
        user_info_by_id, ship_info_by_id = await ship.get_inspect_ship_for_user(
            user_name_or_id,
        )
        if user_info_by_id:
            user_type = user_info_by_id.get("UserType")
            user_name = user_info_by_id.get(USER_DESCRIPTION_PROPERTY_NAME)
            if user_type != "Banned" and user_name:
...
def __create_user_details_from_info(
    user_info: EntityInfo,
    fleet_info: EntityInfo = None,
    ship_info: EntityInfo = None,
    max_tourney_battle_attempts: int = None,
    retrieved_at: datetime = None,
    is_past_data: bool = None,
    is_in_tourney_fleet: bool = None,
) -> entity.EscapedEntityDetails:
    return entity.EscapedEntityDetails(
        user_info,
        __properties["title"],
        None,
...
__properties: entity.EntityDetailsCreationPropertiesCollection = {
    "title": entity.EntityDetailPropertyCollection(entity.EntityDetailProperty("Title", False, omit_if_none=False, transform_function=__get_user_name)),
