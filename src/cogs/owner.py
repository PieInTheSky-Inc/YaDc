...
class OwnerCog(_CogBase, name="Owner commands"):
    ...
    QUERY_UPDATE_SEQUENCES = "\n".join(["DO", "$do$", "BEGIN", "  IF EXISTS (SELECT FROM sales) THEN", "    PERFORM setval('sales_id_seq', (SELECT max(id) FROM sales));", "  END IF;", "END", "$do$"])
    ...
    async def sales_add(self, ctx: _Context, sold_on: str, price: int, currency: str, max_amount: int, *, entity_name: str):
        ...
        self._log_command_use(ctx)
        if price <= 0:
            error_msg = "\n".join([f"Parameter `price` received an invalid value: {price}" f"The value must be greater than 0."])
            raise ValueError(error_msg)

        if max_amount <= 0:
            error_msg = "\n".join([f"Parameter `max_amount` received an invalid value: {max_amount}" f"The value must be greater than 0."])
            raise ValueError(error_msg)
        ...
        if currency_lower.startswith("min"):
            ...
            error_msg = "\n".join([f"Parameter `currency` received wrong value: {currency}" "Valid values are: Bux, Gas, Min, Mineral, Mins, Minerals, Starbux"])
            raise ValueError(error_msg)
        ...
        try:
            ...
            error_msg = "\n".join((f"Parameter `sold_on` received an invalid value: {sold_on}", f"Values must be dates in format: yyyy-MM-dd"))
    ...
    async def sales_import(self, ctx: _Context, *, args: str = None):
        ...
        output = [f"Successfully imported file {attachment.filename}."]
        updated_sequences = await _db.try_execute(OwnerCog.QUERY_UPDATE_SEQUENCES)
        if not updated_sequences:
            output.append("Could not update the sequences!")

        if failed_sales_infos:
            output.append(f"Failed to import the following sales infos:")
