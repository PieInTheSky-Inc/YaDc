...
def transform_pss_datetime(*args, **kwargs) -> datetime:
    ...
    result = None
    if dt:
        result = f'{utils.datetime.get_discord_datestamp(dt)}'
    return result
...
def transform_pss_datetime_with_timespan(*args, **kwargs) -> datetime:
    ...
    if dt:
        ...
        result = f'{utils.datetime.get_discord_datestamp(dt, include_time=include_time)} ({utils.datetime.get_discord_timedelta(dt)})'
    return result
