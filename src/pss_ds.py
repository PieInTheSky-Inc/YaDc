...
def get_dropship_text(as_embed: bool = False):
    path = 'SettingService/GetLatestVersion3?languageKey=en'
    raw_text = core.get_data_from_path(path)
    raw_data = core.xmltree_to_dict2(raw_text, None)
    ...
    lines = daily_msg.append('')
    lines.extend(dropship_msg).append('')
    lines.extend(merchantship_msg).append('')
    lines.extend(shop_msg).append('')
...
def _get_dropship_msg(raw_data: dict, char_designs_data: dict, collection_designs_data: dict) -> list:
    ...
    if raw_data:
        ...
        common_crew_info = crew.get_char_info_short_from_id_as_text(common_crew_id, collection_designs_data, char_designs_data)
        hero_crew_info = crew.get_char_info_short_from_id_as_text(hero_crew_id, collection_designs_data, char_designs_data)
...
def _get_merchantship_msg(raw_data: dict, item_designs_data: dict) -> list:
    ...
    if raw_data:
        ...
        for i, cargo_info in enumerate(cargo_items):
            ...
            price = cargo_prices[i]
            result.append(f'{amount} x {item_details}): {price} bux')
    else:
        result.append('-')
    return result
...
def _get_sale_msg(raw_data: dict, char_designs_data: dict, collection_designs_data: dict, item_designs_data: dict, room_designs_data: dict) -> list:
    ...
    sale_items = _convert_sale_item_mask(sale_item_mask)
    sale_quantity = raw_data['SaleQuantity']
    result.append(f'Buy a {sale_items} of Starbux and get {sale_quantity}x')

    sale_type = raw_data['SaleType']
    entity_id = raw_data['SaleArgument']
    if sale_type == 'Character':
        char_details = crew.get_char_info_short_from_id_as_text(entity_id, char_designs_data, collection_designs_data)
        result.extend(char_details)
    elif sale_type == 'Item':
        item_details = item.get_item_details_short_from_id_as_text(entity_id, item_designs_data)
        result.extend(item_details)
    elif sale_type == 'Room':
        room_details = room.get_room_details_short_from_id_as_text(entity_id, room_designs_data)
        result.extend(room_details)
    else: # Print debugging info
        sale_title = raw_data['SaleTitle']
        result.append(f'Sale Type: {sale_type}')
        result.append(f'Sale Argument: {entity_id}')
        result.append(f'Sale Title: {sale_title}')
