...
PSSAPI_CLIENT = pssapi.PssApiClient(
    device_type=pssapi.enums.DeviceType.ANDROID,
    language_key=pssapi.enums.LanguageKey.ENGLISH,
)
...
class Device:
    ...
    async def __login(self) -> None:
        user_login = await PSSAPI_CLIENT.device_login(self.key, settings.DEVICE_LOGIN_CHECKSUM_KEY)

        if not user_login or not user_login.user:
            error_message = dict(user_login).get("errorMessage")
            if error_message:
        ...
        self.__user = dict(user_login.user)
        self.__access_token = user_login.access_token
        self.__set_can_login_until(utils.get_utc_now())
        self.__set_access_token_expiry()
...
def _create_device_checksum(device_key: str, device_type: str, client_datetime: str) -> str:
    result = hashlib.md5(f"{device_key}{client_datetime}{device_type}{settings.DEVICE_LOGIN_CHECKSUM_KEY}".encode("utf-8")).hexdigest()
    return result
